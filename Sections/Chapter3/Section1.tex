...
Se considera una malla cartesiana uniforme en $[-1,1]^2$ de tamaño $N\times N$, con $N=256$. Los nodos se escriben como
\[
x_i=-1+\frac{2i}{N-1},\qquad
y_j=-1+\frac{2j}{N-1},
...
\]
donde $\Delta t$ coincide con el paso espacial de la malla. Esta elección refleja que el detector tiene un número par de muestras y se centra computacionalmente con el índice $N_t/2$.

\subsection*{Generación del sinograma}

El sinograma se genera con la rutina \texttt{radon} de \texttt{scikit-image} \cite{vanderwalt2014scikitimage}, usando geometría circular. En términos del código reproducible, esto corresponde a la opción \texttt{circle=True}. Esta decisión conserva $N_t=N=256$ muestras de detector.
...
\begin{figure}[htbp]
    ...
    \caption{Fantoma geométrico utilizado en los experimentos y sinograma generado con $M=90$ ángulos en geometría circular. El fantoma está contenido en el disco de radio $R=0.9$, y el sinograma constituye el dato de entrada para las reconstrucciones posteriores.}
    \label{fig:chapter3-phantom-sinogram}
\end{figure}

Esta discretización es suficiente para estudiar el comportamiento relativo de los filtros considerados. Los errores introducidos por interpolación, muestreo espacial y muestreo angular forman parte del modelo numérico evaluado y no se corrigen mediante ajustes posteriores de amplitud \cite{kak_slaney1988,natterer2001}.
