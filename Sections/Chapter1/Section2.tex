...
Sean $X\neq\varnothing$ y $Y\neq\varnothing$ espacios vectoriales normados, ambos sobre $\mathbb{R}$ o $\mathbb{C}$. Una función $T:X\to Y$ se llama operador lineal si
\[
...
Sea $\Omega\subseteq\mathbb R^d$ un conjunto no trivial y medible de Borel. En el espacio $L^{2}(\Omega)$ se usa el producto interno
\[
\langle u,v\rangle_{L^{2}(\Omega)}
=
\int_{\Omega} u(x)\overline{v(x)}\,dx,
\]
con la convención análoga en dominios de una variable. La norma inducida está dada por
\[
\|u\|_{L^2(\Omega)}
=
\left(\int_\Omega |u(x)|^2\,dx\right)^{1/2}.
\]
Si $H_{1}$ y $H_{2}$ son espacios de Hilbert y $T:H_{1}\to H_{2}$ es lineal y acotado, su adjunto es el operador $T^{*}:H_{2}\to H_{1}$ caracterizado por
\[
